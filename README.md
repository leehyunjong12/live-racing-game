...
**Backend**
- [ ] 데이터 베이스 및 환경 구축
- [ ] 핵심 엔티티 설계(사용자 정보, 등록 차량, 총상금)
- [ ] 사용자 인증 및 인가 시스템 구축
- [ ] 회원가입 기능 구현
- [ ] 로그인, 로그아웃 및 세션 관리 구현 
- [ ] 잔액 추가 기능 구현
- [ ] 로그인 된 사용자가 여러 대 자동차 등록 기능 구현
- [ ] 등록 차량 수에 따라 잔액에서 차감 및 상금에 추가 기능 구현
- [ ] 실제 시간 5분마다 경주 자동 시작 기능 구현
- [ ] 경주 자동 시작 전, 모든 자동차 위치, 잔여 턴 등 초기화하는 기능 실행
- [ ] 우승자 발생시, 상금 지급, 상금 초기화, 모든 자동차 정보 삭제 기능 구현
- [ ] 특수 노드 진입 시 발생 이벤트 메시지를 수집하여 JSON에 포함

**Frontend**
- [ ] 회원가입 및 로그인, 로그아웃 폼,버튼 구현
- [ ] 로그인 시 화면 상단 닉네임, 보유 잔액 표시
- [ ] 사용자가 등록한 자동차 대수 닉네임 옆 표시
- [ ] 잔액 충전 입력 폼 및 연동 구현
- [ ] 사용자 전용 자동차 등록 폼 구현
- [ ] 경주 시작 10초 전 자동차 등록, 잔액 충전 폼 자동 비성화
- [ ] 누적 총 상금 금액 표시 UI 구현
- [ ] 경기 종료 시, 라운드 영역을 경기 시작까지 남은시간 타이머로 전환
- [ ] 경기 시작 3초전, 화면 중앙에"3,2,1" 애니메이션 구현
- [ ] 경주 시작시, 타이머 영역 다시 라운드 영역으로 전환
